...
export default function FacilitySection() {
  ...
  const highlights = [
    { icon: Factory, label: t("fac.prod.label"), desc: t("fac.prod.desc") },
    { icon: FlaskConical, label: t("fac.lab.label"), desc: t("fac.lab.desc") },
    { icon: Truck, label: t("fac.dist.label"), desc: t("fac.dist.desc") },
    { icon: ShieldCheck, label: t("fac.safety.label"), desc: t("fac.safety.desc") },
  ];
        ...
        <AnimatedSection>
          <div className="text-center mb-10 sm:mb-16 md:mb-20">
            <div className="flex items-center justify-center gap-3 mb-6 sm:mb-8">
              ...
              <span className="section-label">{t("fac.label")}</span>
              <div className="w-8 h-px bg-ocean/40" />
            </div>
            <h2 className="text-[1.75rem] sm:text-3xl md:text-[2.75rem] font-bold text-foreground leading-[1.15] sm:leading-[1.1] max-w-xl mx-auto">
              {t("fac.h2.pre")} <span className="text-gradient">{t("fac.h2.highlight")}</span>
            </h2>
          </div>
        </AnimatedSection>
        ...
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 max-w-6xl mx-auto items-center">
          <AnimatedSection>
            <div className="relative rounded-2xl overflow-hidden aspect-[3/2] sm:aspect-[4/3]">
              <img src={facilityImage} alt="Gudang penyimpanan bahan kimia ARRAD Chemicals" className="absolute inset-0 w-full h-full object-cover" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, transparent 50%, hsl(var(--navy) / 0.5) 100%)" }} />
            </div>
          </AnimatedSection>
          ...
          <AnimatedSection delay={150}>
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {highlights.map((item) => (
                <div key={item.label} className="bg-secondary/40 rounded-xl border border-border/60 p-4 sm:p-6 flex flex-col gap-2 sm:gap-3">
