...
export default function TechnicalServices() {
  ...
  const services = [
    { icon: Activity, title: t("svc.perf.title"), desc: t("svc.perf.desc") },
    { icon: ClipboardCheck, title: t("svc.qc.title"), desc: t("svc.qc.desc") },
    { icon: FileBarChart, title: t("svc.report.title"), desc: t("svc.report.desc") },
    { icon: GraduationCap, title: t("svc.train.title"), desc: t("svc.train.desc") },
  ];
        ...
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-20">
          <AnimatedSection>
            <div className="relative rounded-xl overflow-hidden aspect-[3/2] sm:aspect-[4/3] lg:aspect-auto lg:h-full min-h-[240px] sm:min-h-[320px] lg:min-h-[360px]">
              <img src={servicesImage} alt="Tim teknis melakukan inspeksi sistem pengolahan air di fasilitas industri" className="absolute inset-0 w-full h-full object-cover" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, hsl(var(--navy) / 0.15) 0%, transparent 50%, hsl(var(--ocean) / 0.1) 100%)" }} />
            </div>
          </AnimatedSection>
          ...
          <div>
            <AnimatedSection>
              <div className="flex items-center gap-3 mb-6 sm:mb-8">
                ...
                <span className="section-label">{t("svc.label")}</span>
              </div>
              <h2 className="text-[1.75rem] sm:text-3xl md:text-[2.75rem] font-bold text-foreground leading-[1.15] sm:leading-[1.1] mb-8 sm:mb-12 max-w-lg">
                {t("svc.h2.pre")} <span className="text-gradient">{t("svc.h2.highlight")}</span>
              </h2>
            </AnimatedSection>
              ...
              {services.map((s, i) => (
                <AnimatedSection key={s.title} delay={i * 80}>
                  <div className="group p-5 sm:p-6 rounded-xl bg-card border border-border/50 hover:border-ocean/15 transition-all duration-500 card-elevated h-full">
                    ...
                    <h3 className="text-[14px] sm:text-[15px] font-bold text-foreground mb-2">{s.title}</h3>
                    <p className="text-muted-foreground leading-[1.65] sm:leading-[1.7] text-[12px] sm:text-[13px]">{s.desc}</p>
                  </div>
                </AnimatedSection>
              ))}
            </div>
